...
export async function listFloatingNotes(conversationId: string): Promise<FloatingNote[]> {
  ...
  const result = await db.query<FloatingNoteRow>(
    `SELECT id, conversation_id, kind, x, y, w, rot, text FROM notes
...
export async function addFloatingNote(note: FloatingNote): Promise<void> {
  const db = await getDb()
  await db.query(
    `INSERT INTO notes (id, conversation_id, kind, text, x, y, w, rot)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
    [note.id, note.conversationId, note.kind, note.text, note.x, note.y, note.w, note.rot],
  )
}
